<?php
    ...
    $query = "SELECT * FROM cardio_log WHERE clientID = '$id'";
    ...
    if (!$result) {
        ...
        if ($result_row_count <= 0) {
            ...
            //fetch data
            $cardio_log = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<?php include('inc/clientHeader.php'); ?>
	<div class="container">
		<h1>My Food Logs</h1>
        <?php foreach($cardio_log as $log) : ?>
            <div>
                <h3><?php echo $log['log_name']; ?></h3>
                <h4>date: <?php echo $log['log_date']; ?></h4>
                <h4>cardio type: <?php echo $log['cardio_type']; ?></h4>
